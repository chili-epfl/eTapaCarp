...
View.prototype.saveBricks = function() {
    this.brickManager.saveBricks();

    var data = atob(this.renderer.domElement.toDataURL().substring("data:image/png;base64,".length)),
...
View.prototype.render = function(markers) {
    ...
    if (this.dynamic) {
        ...
        for (var i in markers) {
            ...
            //Here the "if" to avoid the detections of the tools or
            //other things as bricks
            if (Session.get('shapes')[marker.id]) {
...
View.prototype.clear = function() {
    ...
    for (var i in this.toolManager.tools) {
        this.toolManager.tools[i].changeVisibility(false);
    }
...
View.prototype.createObjects = function(markerId) {
    if (!this.brickManager.bricks[markerId]) {
        ...
        $('#shapes').append("<h6 id=\"Shape" + brick.id + "head\"> Shape " + brick.id + "</h6>" + "<div id=\"Shape" + brick.id + "\" class=\"btn-toolbar\"></div>");
        $('#Shape' + brick.id).append("<button id=\"All\" class=\"btn btn-mini btn-primary\" style=\"margin-top:5px;\">All</button>");
        $('#Shape' + brick.id).append("<button id=\"Block0\" class=\"btn btn-mini\" style=\"margin-top:5px;\">" + 0 + "</button>");
        $('#Shape' + brick.id).append("<div id=\"actions\" class=\"btn-toolbar text-center\"></div>")
        $('#Shape' + brick.id + '>' + '#Block0').css({'background-image': 'none', 'background-color': (brick.HTMLcolors[0])});

        (function(brick, view) {
            $('#Shape' + brick.id + '>' + '#Block0').on('click', $.proxy(function() {
                if (brick.visibility) {
                    $('#Shape' + brick.id + '>' + "button[id*='Block']").removeClass('btn-primary');
                    $('#Shape' + brick.id + '>' + "button[id*='All']").removeClass('btn-primary');
                    $('#Shape' + brick.id + '>' + '#Block0').addClass('btn-primary');
...
//                        view.renderer.render(view.scene, view.camera);
        ...
        (function(brick, view) {
            $('#Shape' + brick.id + '>' + '#All').on('click', $.proxy(function() {
                if (brick.visibility) {
                    $('#Shape' + brick.id + '>' + "button[id*='Block']").removeClass('btn-primary');
                    $('#Shape' + brick.id + '>' + "button[id*='All']").removeClass('btn-primary');
                    $('#Shape' + brick.id + '>' + '#All').addClass('btn-primary');
                    brick.highlightBlock();
                    //view.renderer.clear();
//                    if (Session.get("virtual") == true)
                    MarkersDetector.forceUpdate = true;
...
//                        view.renderer.render(view.scene, view.camera);
        ...
        $('#Shape' + brick.id + '>' + '#actions').append("<button id=\"delete-shape\" class=\"btn btn-mini\"  style=\"margin-top:5px;\">Delete</button>");



        (function(brick, view) {
            $('#Shape' + brick.id + '>' + '#actions' + '>' + '#delete-shape').on('click', $.proxy(function() {
                if (brick.visibility) {
                    view.removeBrickFromScene(brick);
                    brick.deleteBlock();
                    view.addBrickToScene(brick);
                    //$('#Shape' + brick.id + '> #restore-shape').text("Undo(" + brick.removedBlocks.length + ")");
//                    if (Session.get("virtual") == true)
                    MarkersDetector.forceUpdate = true;
//                    else
//                        view.renderer.render(view.scene, view.camera);
        ...
        $('#Shape' + brick.id + '>' + '#actions').append("<button id=\"restore-shape\" class=\"btn btn-mini\" style=\"margin-top:5px;\">Undo</button>");


        (function(brick, view) {
            $('#Shape' + brick.id + '>' + '#actions' + '>' + '#restore-shape').on('click', $.proxy(function() {
                if (brick.visibility) {
                    view.removeBrickFromScene(brick);
                    var restoredId = brick.restoreBlock();
                    if (restoredId != null && restoredId >= 0) {
                        $('#Shape' + brick.id + ">*").each(function(index, Element) {
                            if (
                                    (function(restoredId, Element) {
                                        var elemName = Element.attr('id');
                                        if (elemName == "All")
                                            return 1;

                                        if (elemName == "delete-shape" || elemName == "actions")
                                            return -1;
                                        elemName = elemName.replace("Block", '');
                                        elemName = parseInt(elemName);
                                        if (restoredId > elemName)
                                            return 1;
                                        else
                                            return -1;
                                    })(restoredId, $(this)) < 0) {
                                $(Element).before("<button id=\"Block" + restoredId + "\" class=\"btn btn-mini\" style=\"margin-top:5px;\">" + restoredId + "</button>");
                                return false;
                            }
                        ...
                        //$('#Shape' + brick.id + "> #delete-shape").before("<button id=\"Block" + restoredId + "\" class=\"btn btn-mini\">" + restoredId + "</button>");
                        $('#Shape' + brick.id + '>' + '#Block' + restoredId).css({'background-image': 'none', 'background-color': (brick.HTMLcolors[restoredId % brick.colors.length])});

                        $('#Shape' + brick.id + '>' + '#Block' + restoredId).on('click', $.proxy(function() {
                            $('#Shape' + brick.id + '>' + "button[id*='Block']").removeClass('btn-primary');
                            $('#Shape' + brick.id + '>' + "button[id*='All']").removeClass('btn-primary');
                            $('#Shape' + brick.id + '>' + '#Block' + restoredId).addClass('btn-primary');
                            brick.highlightBlock();
                            //view.renderer.clear();
//                            if (Session.get("virtual") == true)
                            MarkersDetector.forceUpdate = true;
...
//                                view.renderer.render(view.scene, view.camera);
                    ...
                    else if (restoredId != null && restoredId < 0) {
                        $('#Shape' + brick.id + '>' + "button[id*='Block']").each(
                                function(index, Element) {
                                    ...
                                    for (var i in brick.blocks) {
                                        if (parseInt(brick.blocks[i].id) == name) {
                                            flag = true;
                                            break;
                                        }
                                    }
...
//                        view.renderer.render(view.scene, view.camera);
        ...
        this.brickManager.bricks[markerId].setRotationAndTranslation(0, {x: 0, y: 0},false);
    }
};

/*The only function concerning the tools*/
View.prototype.createTools = function(markers) {
    var toolsCollection = Session.get('tools');
    for (var t in toolsCollection) {

        var tool = toolsCollection[t];

        /*Tool is on the Work Space flag*/
        var toolOnWS = true;
        for (var i = 0, tot = tool.markers.length; i < tot; i++) {
            if (!markers.hasOwnProperty(tool.markers[i])) {
                toolOnWS = false;
                break;
            }
        }
        ...
        if (toolOnWS) {
            if (!this.toolManager.tools[tool.id]) {
                //Creation
                var toolFactory = new ToolFactory();
                var new_tool = toolFactory.createTool(tool.id, this.brickManager, markers);
                this.toolManager.addTool(new_tool);
                this.scene.add(new_tool.mesh);
                if (tool.id == 83)
                    this.scene.add(new_tool.floorLine)

            }
            else {
                //Reset
                this.toolManager.tools[tool.id].reset();
            }
            //Update of the status
            this.toolManager.tools[tool.id].changeVisibility(true);
            this.toolManager.tools[tool.id].computeNewPosition(markers);
            this.toolManager.tools[tool.id].computeNewStatus(markers, this);
        }
...
View.prototype.computeNewPositions = function(markers) {
    ...
    for (var k in this.brickManager.bricks) {
        ...
        for (var l in markers) {
            ...
            if (k == marker.id) {
                var Z = brick.markerZ;
                //MArker.corners.size=4, here it takes the first 2 corners, which
                // lays on the x axis??? and compute the position
                var marker_position = [marker.corners[0].x, marker.corners[0].y, 1];
                var marker_position2 = [marker.corners[1].x, marker.corners[1].y, 1];
                var position;
                var position2;
                if (this.virtual) {
//                    console.log(brick.centroid.x, brick.centroid.y);
                    position = {x: marker_position[0], y: marker_position[1]};
                    position2 = {x: marker_position2[0], y: marker_position2[1]};
                var rotation = this.calculateRotation(position, position2);
                    var v = new THREE.Vector3(brick.centroid.x, brick.centroid.y, 0);
                    v.applyAxisAngle(new THREE.Vector3(0, 0, 1), rotation);
                    brick.setRotationAndTranslation(rotation, {x: marker_position[0] - v.x, y: marker_position[1] - v.y},true);
            }
                else {
                    position = pixel2mm(marker_position[0], marker_position[1], Z);
                    var test = mm2pixel(position.x, position.y, Z);
                    position2 = pixel2mm(marker_position2[0], marker_position2[1], Z);
                    var rotation = this.calculateRotation(position, position2);
                    brick.setRotationAndTranslation(rotation, {x: position.x, y: position.y},true);
        }
...
View.prototype.calculateRotation = function(vector1, vector2) {
    ...
    rotation = Math.round(rotation / 0.03490) * 0.03490;
...
View.prototype.showAxis = function() {
    ...
    /*New plane to hide z<0 objects*/
    material = new THREE.MeshBasicMaterial({color: 0xFFFFFF, side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1});
    line = new THREE.Geometry();
    line.vertices.push(new THREE.Vector3(WS_WIDTH, WS_HEIGHT, -0.15));
    line.vertices.push(new THREE.Vector3(-WS_WIDTH, WS_HEIGHT, -0.15));
    line.vertices.push(new THREE.Vector3(-WS_WIDTH, -WS_HEIGHT, -0.15));
    line.vertices.push(new THREE.Vector3(WS_WIDTH, -WS_HEIGHT, -0.15));
    line.faces.push(new THREE.Face3(0, 1, 2));
    line.faces.push(new THREE.Face3(0, 2, 3));
    line.computeFaceNormals();
    var object7 = new THREE.Mesh(line, material);
    //this.axisObjects.push(object7);
...
PerspectiveView.prototype.setCameraWidth = function(width) {
    this.width = width;
    this.height = this.width / (180 / 140);
    this.camera = new THREE.PerspectiveCamera(50, WS_WIDTH / WS_HEIGHT, 1, 1000);
    this.camera.position.x = 0;
    this.camera.position.y = -300;
    this.camera.position.z = 200;
...
PerspectiveView.prototype.zoomIn = function() {
    if (this.camera.fov > 10) {
        this.camera.fov = this.camera.fov / 1.1;
...
PerspectiveView.prototype.zoomOut = function() {
