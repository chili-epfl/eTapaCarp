...
Template.activity4Difficulty.events({
    'click a[id^="shape"]': function(e, tmpl) {
        shape = e.target.id.split('shape')[1];
        Meteor.Router.to('/activity4/practice/ready');
    }
});
Template.activity4.lang = function(e) {
    lang = Session.get('lang');
    return lang;
}

Template.activity4.isPractice = function() {
    isPractice = Session.get('isPractice')
    return isPractice;
}

Template.activity4.rendered = function() {
    ...
    if (!rendered) {
        ...
        var name = "Activity4-" + Meteor.user().emails[0].address+ "Shape:" + shape;
        ...
        Logger.userId = name;
        Logger.postTag("Activity4 started;Shape:"+shape);
        Logger.postEvent("Activity4 started;Shape:"+shape);
        Logger.postBrick("Activity4 started;Shape:"+shape);
        ...
        activity.template = Template.activity4;
...
Template.activity4.updateTime = function() {
    if (timer) {
        var endTime = (new Date().getTime() - startTime) / 1000.0;
        $('#time').text(endTime);
        timer = setTimeout(Template.activity4.updateTime, 100);
    }
}

Template.activity4.activityFinished = function() {
    if (!isPractice) {
        timer = null;
...
function createNextActivityHandler() {
    $("#nextActivityButton").on('click', function() {
        ...
        newDiff = Math.min(activity.difficulty + 1, Config.Activity4.MAX_DIFFICULTY);
...
Template.activity4.startActivity = function(markerId) {
    viewManager.edgeToSelect(markerId, activity.difficulty, 'perspective');
    startTime = new Date().getTime();
    scoreId = Score.insert({time: null, activity: "activity4", userId: Meteor.userId(), date: new Date(), difficulty: activity.difficulty, shape: markerId});
    timerStarted = true;
    timer = setTimeout(Template.activity4.updateTime, 100);
    activity.update(markersDetector);
}

Template.activity4.destroyed = function() {
    viewManager.destroy();
